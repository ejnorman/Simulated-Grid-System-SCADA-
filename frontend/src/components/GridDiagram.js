...
function buildStatusMaps(alarms, metrics) {
  ...
  for (const line of metrics?.lines ?? []) {
    lineService[line.id] = { in_service: line.in_service, loading_percent: line.loading_percent };
  }
...
export default function GridDiagram({ metrics, alarms }) {
  const { lineStatus, busStatus, lineService, genService } = buildStatusMaps(alarms, metrics);

  return (
    <Paper sx={{ p: 2, height: '100%' }}>
      <Typography variant="h6" gutterBottom>IEEE 14-Bus System</Typography>
      ...
      <svg width="100%" height="22" style={{ marginBottom: 6 }}>
        {LEGEND.map(({ color, label, circle, dashed }, i) => (
          <g key={label} transform={`translate(${i * 115 + 8}, 4)`}>
            ...
            <text x="19" y="11" fontSize="10" fill="#888">{label}</text>
          </g>
        ))}
      </svg>
      ...
      >
        ...
        {LINE_TOPOLOGY.map(({ id, from, to }) => {
          ...
          const color   = tripped ? LINE_TRIPPED : (sev ? ALARM_COLOR[sev] : LINE_NORMAL);
        ...
        {LINE_TOPOLOGY.map(({ id, from, to }) => {
          ...
          const idColor  = tripped ? '#777' : (sev ? ALARM_COLOR[sev] : '#555');
          const subLabel = tripped ? 'OPEN' : (loading > 1 ? `${loading.toFixed(0)}%` : null);

          return (
            <g key={`lid-${id}`}>
              <text
                x={mx} y={my - (subLabel ? 6 : 4)}
              ...
              {subLabel && (
                <text
                  ...
                  fill={tripped ? '#777' : (sev ? ALARM_COLOR[sev] : '#666')}
                  fontWeight={sev && !tripped ? 'bold' : 'normal'}
                >
                  {subLabel}
                </text>
        ...
        {Object.entries(BUS_POSITIONS).map(([busIdStr, { x, y }]) => {
          ...
          return (
            <g key={busId}>
              ...
              <text x={x} y={y - r - 4} textAnchor="middle" fontSize="11" fontWeight="bold"
